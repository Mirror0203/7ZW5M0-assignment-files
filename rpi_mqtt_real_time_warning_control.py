# -*- coding: utf-8 -*-
"""
Created on Mon Nov 29 22:44:40 2021
...
def on_message(client, userdata, message):
    ...
    # the thresholds will be changed in this section
    change=message.payload.decode()
    threshold_change=[int(s) for s in re.findall(r'\b\d+\b', change)] #Assign values to three threshold-variables
    # if the thresholds is changed, an alarm will also be published to xxx/warning
...
while True:
    ...
    try:
        ...
        if aq_value > highair_td: # to detect if the aq value is above the high pollution threshold value
        ...
        elif aq_value>lowair_td: # to detect if the aq value is above the low pollution threshold value
        ...
        else:  # if the air is fresh, no warning will be sent
            msg_2='Fresh air:' + str(aq_value)
        print(msg_2)


        if sound_value>noisy_td: # to detect if the sound value is above the noisy threshold value
